...
import { deleteToken, redirectServerAction, redirectToPrev } from "@/app/lib/cookie";
    ...
    <form action={formAction}>
      {!state.submitted ? (
        <BasicConfirm onCancel={()=>setVisible(false)}>
            ...
            <div className="flex justify-center" ><span className="text-red-warning ">모든 정보가 삭제</span>됩니다.</div>
            </div>
          
        </BasicConfirm>
      ) : (
        <BasicAlert onClick={()=>{redirectServerAction('/login')}}>
            {
